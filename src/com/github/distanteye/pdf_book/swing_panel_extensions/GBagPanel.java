...
import com.github.distanteye.pdf_book.spe_wrappers.AccessWrapper;
import com.github.distanteye.pdf_book.spe_wrappers.ComboBoxDDMappedComponent;
import com.github.distanteye.pdf_book.spe_wrappers.MappedComponent;
import com.github.distanteye.pdf_book.spe_wrappers.MappedComponent.DataFlow;
...
public class GBagPanel extends ExtJPanel {			
	 ...
	 * @param y nonzero int
	 */
	public JLabel endVertical(int x, int y)
	{ 
		cons.weighty = 100.0;
		JLabel result = addLabel(x,y,"");
		cons.weighty = 1.0;
		return result;
	}
	
	/**
	 * Adds a JLabel JComboBox pair (each weight 1 ), and adds it the mappedComponents list. These are always pull model for populating the dropdown
	 * using the labelText for a key.
	 * @param editState Whether the Combobox is editable or fixed
	 * @param x non-negative integer
	 * @param y non-negative integer
	 * @param labelText Label to display
	 * @param mapName The key for the ComboBox in MappedComponents
	 * @param o Whether the label/textfield pair is oriented horizontally or vertically
	 * @param accessWrapper valid AccessWrapper that links to the data element the ComboBox will pull options from
	 * @param dropDownOptions things to prepopulate the dropdown with
	 * @param separator If joining (almost always doing this) data for the pull, the data is passed as a flat string, separator is what to know to split on
	 * 
	 * @return The component created
	 */
	public JComboBox<String> addMappedComboBox(EditState editState, int x, int y, String labelText, String mapName,  
									Orientation o, AccessWrapper<String> accessWrapper, String[] dropDownOptions, String separator)
	{
		int newX,newY;
		...
		JComboBox<String> temp = new JComboBox<String>(dropDownOptions);
		addC(temp,newX,newY);
		...
		this.putMapped(mapName, new ComboBoxDDMappedComponent(df,accessWrapper,temp,separator));
