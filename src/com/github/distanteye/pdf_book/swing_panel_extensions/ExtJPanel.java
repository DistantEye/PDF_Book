...
public class ExtJPanel extends JPanel {
	...
	protected double oldXWeight, oldYWeight; // for storing when temporarily setting a new weight 
	protected boolean differentWeights;
	...
	/**
	 * Helper method, searches mappedComponents for a ComboBox with name, and returns the selected text (if any)
	 * 
	 * @param name Valid name 
	 * @return The selected text for the ComboBox, or "", if nothign is selected
	 */
	public String getComboBoxText(String name)
	{
		...
		else
		{
			...
			{
				Object res = ((JComboBox<?>)temp).getSelectedItem();
				if (res == null) { 
					return ""; 
				}
				else { 
	...
	public ExtJPanel setNextWeights(double x, double y)
	{
		oldXWeight = cons.weightx;
		oldYWeight = cons.weighty;
		cons.weightx = x;
		cons.weighty = y;
		this.differentWeights = true;
